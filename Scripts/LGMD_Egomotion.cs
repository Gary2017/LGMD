...
public class LGMD_Egomotion : MonoBehaviour
{
    // global frames count
    ...
    private bool moveState;  // ÒÆ¶¯×´Ì¬
    private bool rotateState;  // ×ªÏò×´Ì¬
    ...
    void Start()
    {
        ...
        c_width = 100;
        c_height = 100;
        Locust = new LGMD2(c_width, c_height, 50);
        moveState = true;
        rotateState = false;
    ...
    void FixedUpdate()
    {
        ...
        Locust.LGMD2_Processing(globalFrames + 1);
        globalFrames++;

        if (moveState)
        {
            transform.Translate(Vector3.forward * 2 * Time.fixedDeltaTime, Space.Self);
            /*ReadFromCamera(LocustEye, ref Locust.seen);
            Locust.LGMD2_Processing(globalFrames + 1);
            globalFrames++;*/
        }
    ...
    public class LGMD2
    {
        ...
        public LGMD2(int width/*frame width*/, int height /*frame height*/, int fps /*frames per second*/)
        {
            //stimuli property
            this.width = width;
            this.height = height;
        ...
        /// <param name="t"></param>
        public void LGMD2_Processing(int t)
        {
            //init
            int cur_frame = t % 2;
            int pre_frame = (t - 1) % 2;
            ...
            for (int y = 0; y < height; y++)
            {
                ...
                {
                    photoreceptors[y, x, cur_frame] = HighpassFilter(seen[y, x, cur_frame], seen[y, x, pre_frame]);
